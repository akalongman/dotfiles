...
- Before recommending an action that rests on a factual claim from a project's CLAUDE.md or docs (where data lives, which host owns what, what a mechanism carries), verify the claim on the live system when a read-only check is cheap. Project docs describe the design as it was written and drift silently from what the deployment actually does; a wrong claim from docs, delivered confidently, costs a full correction cycle (2026-09-11: recommended backing up LMS upload directories the docs described, which were empty because the apps had moved uploads to S3).
- Before writing a Dockerfile layer that installs packages on top of a third-party image (`RUN apk add`, `apt-get install`, `pip install`), run the pinned image once and confirm the package manager exists (`docker run --rm <image> sh -c 'command -v apk apt-get pip'`). Hardened and distroless images strip it at the runtime stage while the project's own Dockerfile still shows it in build stages, so reading the upstream Dockerfile proves nothing. The check takes seconds; skipping it costs a failed first deploy (2026-09-11: planned `apk add ffmpeg` on the n8n 2.x image, which ships without `apk`; static binaries copied from a build stage were the fix).
- When a conclusion depends on whether something is present or absent (auth status, a config key, a list entry, an installed tool, a flag), read the command's full output before asserting. Do not pipe a status or diagnostic command through `tail`, `head`, or a narrow `grep` and then conclude something is missing. The hidden lines are often exactly the ones that decide the answer. Reserve truncation for genuinely bulky output whose relevant portion sits at a known location. The same applies to argument-scoped queries (`git ls-files <paths>`, `ls <dir>`): output scoped to specific paths proves nothing about paths outside its arguments; before asserting something is untracked or absent, run the query that would actually show it. The same applies to diffs, in the opposite direction: do not conclude a line is new from `grep '^+'` output. Whitespace or alignment changes re-emit unchanged lines as `-`/`+` pairs, and filtering to the `+` side removes the very evidence that would show the line already existed. Read the full hunk with context before asserting something was added, and check which enclosing block or property the change actually lands in.
- A failed auth or connectivity check proves only the credential source it exercised (one profile, one set of env vars, one endpoint). Before saying a tool "does not work", enumerate the other sources it resolves (env vars, other profiles, endpoint overrides, project-local config) and state precisely which one failed (2026-09-12: called the AWS CLI "dead" on both machines because the stored default key was invalid, while the actual workflow used env vars against a Garage endpoint and worked).
...
- When a test run fails, establish whether the cause is the code or the test environment BEFORE naming a cause, and prefer a cheap direct measurement over a plausible story. The signature of polluted shared state is a scatter of failures across unrelated areas, typically list or count assertions on reference tables (degrees, currencies, statuses), with no connection to what changed. Measure it directly: query the committed row counts of those tables from a separate connection and compare against what a freshly migrated database should hold (usually zero, when seeders are not in the test path). Rows are visible from another connection only if they were committed, so a non-zero count IS the proof. Killed runs cause this, but so do runs that finish normally: assume every full-suite run leaves some committed residue, and reset the test database before any run whose result you intend to act on, not only after something went wrong. Reset and re-run before touching application code. Corollary discipline, each of which cost a wrong diagnosis at least once: never truncate a failing run's output (the failure messages are the evidence); read EVERY failure before naming a cause, because N failures are not automatically one cause, and generalising from the first stack trace produces a confident diagnosis and a fix for a problem that was never there (when a proposed fix is acted on, re-read the new failures rather than assuming the remaining ones are the old ones); never edit source while a validation run is in flight, because the runner loads classes from the working tree as it goes, so tests before and after an edit run different code; and verify claimed shared state instead of recalling it, since per-worktree or per-job database cloning may already isolate what a stored note says is shared.
- A regression test can only turn red on a decision made by the code it drives. Before designing a reproduction, name the exact boundary where the wrong value is chosen (the caller that hands over leaked state, the request that picks the wrong tenant) and drive THAT unit; if the test supplies the disputed value itself (a probe fed its target, a service fed its scope), the defect sits outside the test's reach and the test is green on buggy code. This also decides commit order: a red-then-green sequence needs the reproduction against unchanged code first, never a behaviour-preserving extraction that changes the tested constructor between red and green (2026-09-12: designed a probe-level infra test for sc-843 that could never fail, because the test chose the probe target; the reproduction had to drive the refresh command with a leaking fake callee instead).
- When recommending between design variants, weight simplicity and the codebase's existing idioms above expressiveness. Before adding a new axis (a per-grant binding, a new mapping, a parallel subsystem) to carry a requirement, check whether an existing context object the system already resolves per request (account, semester, scope value object) can absorb it; prefer the variant that leaves existing structures untouched. Buy expressiveness only for a named, present-day use case, not for a hypothetical persona.
- When creating a temporary or working document file (a report, analysis, plan, design note, draft, or scratch markdown/HTML artifact, whether in the scratchpad or inside a project), name it with the date and time first and the descriptive name only after, in 24-hour local time: `YYYY-MM-DD-HHMM-<name>.<ext>`, for example `2026-09-04-1532-erp-audit-report.md`. The prefix makes a directory of such files sort chronologically and keeps successive versions of the same document from overwriting each other. This does not apply to source code, permanent project documentation (README, `docs/dev/`), or files whose name is dictated by tooling (migrations, OpenSpec change directories).
...
- When working with RabbitMQ or message queues, invoke the `rabbitmq-guidelines` skill (moved out of ~/.claude/rules/ so it loads on demand instead of in every session)
