...
**REQUIRED SUB-SKILL:** superpowers:subagent-driven-development runs the loop: dispatch, task review, fix rounds, ledger, final review. Invoke it with the Skill tool on every invocation of this skill (after a `/clear` nothing is loaded), unless its full text is already in this transcript. This skill adds only the source adapter, this machine's constraints, and six overrides that each fix a failure measured in real runs.
...
**OpenSpec.** Load with `openspec instructions apply --change <name> --json | jq '{state, progress, tasks, instruction}'`, never raw. `blocked`: print the instruction, stop. `all_done`: render the table, suggest `/opsx:archive`, stop. `ready`: read `tasks.md` and the spec deltas once for the pre-flight scan; proposal and design reach implementers as paths. The instruction's "mark complete as you go" is superseded: the controller ticks after review. Command-only items run in the controller and tick when they pass; prose-only edits are the controller's and the final review covers them; anything executable or declarative goes to an implementer; an external side effect (push, merge request, ticket) stops for the user. A task that can only run after merge (it needs the target branch, the default branch's configuration, or a later merge request) is a source defect: the pre-flight scan flags it, and it is amended with the user into a pre-merge task that writes it to the merge request description's `### Post-merge checks` list. It is never parked as an open task, because the change archives before its merge request merges. A code-to-source mismatch stops the loop: amend the source with the user, `openspec validate <name> --strict`, regenerate the brief. At the end, render what `instruction` asks, suggest `/opsx:archive`, and delete `<ws>` only after the archive.
...
4. **Models.** Aliases `sonnet`, `opus`, `fable`; the tier per role is override 6, not a choice. Before the first dispatch, ledger `Model policy: implementer opus, reviewer opus, final review fable` plus any unit the user named for `sonnet`. `fable` is capped on this account: the final review and rounds 4 and 5 only. On `rate_limit`, `429` or `reached your <tier> limit`: ledger `Model fallback: <role> <from> -> <to>`, run that role one tier down for the rest of the session, never re-probe.
...
6. **Tiers are fixed.** Implementers and every reviewer (task review, re-review, final-fix re-review) dispatch on `opus`; the final review and fix rounds 4 and 5 on `fable`, with item 4's fallback; `sonnet` only for a unit the user names. The sub-skill's Model Selection does not apply here: controllers reading it put prose-specified units and their reviews on the cheapest tier, and those implementers came back at 445k and 490k tokens with the most findings (2026-09-23).
